...
module "addresses" {
  for_each   = toset([for v in var.network_interfaces : v.subnetwork if v.allocate_external_address])
  source     = "github.com/dapperlabs-platform/terraform-google-net-address?ref=v0.9.0"
  project_id = var.project_id
  external_addresses = {
    "${var.name}" = local.region
  }
}
...
resource "google_compute_firewall" "allow_rdp" {
  ...
  source_ranges = var.allow_rdp_ranges
...
resource "google_compute_instance" "default" {
  ...
  dynamic "network_interface" {
    ...
    content {
      ...
      dynamic "access_config" {
        ...
        content {
          nat_ip = try(module.addresses[config.value.subnetwork].external_addresses[var.name].address, null)
        }
      }
  ...
  scheduling {
    automatic_restart   = try(var.scheduling.automatic_restart, !var.options.spot)
